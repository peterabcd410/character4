...
class RefCOCODataSet(Data.Dataset):
    def __init__(self, 
        ...
        self.ques_list = []
        splits=split.split('+')
        print(splits)  # 打印['train']
        self.refs_anno=[]  #从总JSON中按数据的形式存放 训练集 或 测试集 的annotations  
        ...
        # 在训练集或测试集的ann中  取 'refs'字段的数据 添加到数据 refs[]中
    ...
    def __getitem__(self, idx):
        ...
        print('ref_iter  ')
        print(ref_iter) #[139  16  17   5 294  73 162 127   0   0   0   0   0   0   0]
        image_iter,mask_iter,gt_box_iter,mask_id,iid= self.load_img_feats(idx)
        print('image_iter')
        print(image_iter) #image arr
        print('mask_iter')
        print(mask_iter)# 0 arr 
        print('gt_box_iter')
        print(gt_box_iter) # box zuobiao
        print('mask_id')
        print(mask_id) # maskid 1907
        print('iid')
        print(iid) # iid 561323
